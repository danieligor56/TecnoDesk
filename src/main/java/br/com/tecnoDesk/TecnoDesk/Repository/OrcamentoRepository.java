...
public interface OrcamentoRepository extends JpaRepository<Orcamento, Long> {
	...
	@Query(value = "SELECT * FROM Orcamento c WHERE c.codigo_empresa = :CodEmp and c.id = :codOrcamento", nativeQuery = true)
	Orcamento encontrarOcamentoPorID(long CodEmp, long codOrcamento);

	@Query(value = "SELECT * FROM Orcamento c WHERE c.codigo_empresa = :CodEmp and c.codigo_os = :numOs", nativeQuery = true)
	Orcamento encontrarOcamentoPorNumOS(long CodEmp, long numOs);

	@Modifying
	@Transactional
	@Query(value = "DELETE FROM orcamento_item " +
			"WHERE id = :idItemOrcamento " +
			"AND codigo_orcamento = :codigoOrcamento " +
			"AND codigo_empresa = :CodEmp", nativeQuery = true)
	...
	// Dashboard query
	long countByEmpresaIdAndStatusOR(Long empresaId, StatusOR statusOR);
