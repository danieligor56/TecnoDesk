...
public class ProdutoService {
	...
	public Produtos criarNovoProduto(ProdutosDTO dto, String codEmpresa) {
		try {
			...
			// Se código de barras não foi informado (0 ou negativo), gera automaticamente
			// no padrão EAN-13
			...
			if (dto.getCategoria() != null) {
				novoProduto.setCategoria(categoriaProdutoRepository.findById(dto.getCategoria()).orElse(null));
			}
			if (dto.getUnidadeMedida() != null) {
				novoProduto.setUnidadeMedida(unidadeMedidaRepository.findById(dto.getUnidadeMedida()).orElse(null));
			}
			...
			try {
				ControleEstoqueDTO controleEstoqueDTO = new ControleEstoqueDTO(novoProduto.getId(), BigDecimal.ZERO,
						BigDecimal.ZERO, "");
				...
				throw new BadRequest(
						"Não foi possível cadastrar o produto no estoque, por favor.adicione de forma manual"
								+ e.getMessage());
			}
	 ...
	 * Formato: 789 (Brasil) + 9 dígitos (ID produto + código empresa) + dígito
	 * verificador
	 * 
	...
	private long gerarCodigoBarrasEAN13(long idProduto, long idEmpresa) {
		...
		String empresaPart = idEmpresaStr.length() > 3 ? idEmpresaStr.substring(idEmpresaStr.length() - 3)
				: String.format("%03d", Long.parseLong(idEmpresaStr));

		// Pega até 6 dígitos do ID do produto (do final)
		String produtoPart = idProdutoStr.length() > 6 ? idProdutoStr.substring(idProdutoStr.length() - 6)
	...
	public Produtos alterarProduto(long id, ProdutosDTO dto, String codEmpresa) {
		try {
			...
			if (produtoExistente != null) {
				...
				if (dto.getCategoria() != null) {
					produtoExistente.setCategoria(categoriaProdutoRepository.findById(dto.getCategoria()).orElse(null));
				} else {
				...
				if (dto.getUnidadeMedida() != null) {
					produtoExistente
							.setUnidadeMedida(unidadeMedidaRepository.findById(dto.getUnidadeMedida()).orElse(null));
				} else {
					produtoExistente.setUnidadeMedida(null);
				}
